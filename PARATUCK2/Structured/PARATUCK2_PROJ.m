...
%taking into account the neural network structure.
    ...
    for i=1:100
        ...
        % Projectie strategie
        [cD2, D2] = update_cD2(D2, Vt, Zt, samples, cD1, bf1, bf2, K, r2, d2);
        ...
        % Projectie strategie
        [cD1, D1] = update_cD1(D1, Zt, samples, bf1, K, r1, d1);
        ...
        if(error < 0.01 || abs(error-lastError) < 0.00005)
            break
        end
...
function [cD1, D1] = update_cD1(D1, Zt, samples, bf1, K, r1, d1)
    % Projectiestrategie
    X = zeros(r1*K, r1*d1);
    for l=1:r1
        Xl = zeros(K, d1);
...
function [cD2, D2] = update_cD2(D2, Vt, Zt, samples, cD1, bf1, bf2, K, r2, d2)
    % Projectiestrategie
    X = zeros(r2*K, r2*d2);
    for l=1:r2
        Xl = zeros(K, d2);
